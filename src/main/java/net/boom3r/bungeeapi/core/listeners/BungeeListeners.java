...
public class BungeeListeners implements Listener {
    ...
    public void onPostLogin(PostLoginEvent event) {
        ...
        if (whitelistEnabled) {
            if (WhiteListManager.isWhiteListed(event.getPlayer())) {
                sendWelcomeMessage(event.getPlayer().getName());
    ...
    private static void sendWelcomeMessage(String name){
        for (ProxiedPlayer player : ProxyServer.getInstance().getPlayers()) {
            if (!player.getName().equalsIgnoreCase(name)){
                player.sendMessage(new TextComponent(name + " a rejoins le network !"));
            }
